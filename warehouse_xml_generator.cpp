...
void AddTransition(tinyxml2::XMLNode* component, int& transitionLabelCounter, int source, int target, int rowSize, int colSize, const std::map<int, SpecialLocationType>& specialLocations) {
    ...
    if (isHorizontal) {
        ...
        guardCondition = "x1==" + std::to_string(sourceCol) + " & x2>=" + std::to_string(sourceRow) + " & x2<" + std::to_string(sourceRow + 1);
    } else if (isVertical) {
        // Vertical transition
        guardCondition = "x1>=" + std::to_string(sourceCol) + " & x1<" + std::to_string(sourceCol + 1) + " & x2==" + std::to_string(sourceRow);
    }
...
//void GenerateXml(int gridSize, const std::vector<int>& specialLocations) {
void GenerateXml(int rowSize, int colSize, const std::map<int, SpecialLocationType>& specialLocations) {
    ...
    /*for (int row = 0; row < gridSize; ++row) {
        for (int col = 0; col < gridSize; ++col) {
            ...
            bool isSpecial = std::find(specialLocations.begin(), specialLocations.end(), id) != specialLocations.end();
            AddLocation(component, id, row, row + 1, col, col + 1, isSpecial);
        }
    }*/
...
for (int row = 0; row < rowSize; row++) {
    for (int col = 0; col < colSize; col++) {
        ...
        if (row < rowSize - 1) { // Note: 'row > 0' because row 0 is the topmost row
...
void generateCfgFile(const std::map<int, SpecialLocationType>& specialLocations, const std::string& filename, int rowSize, int colSize) {
    ...
    auto findValidLocation = [&](SpecialLocationType avoidType, int excludeLocation = -1) -> int {
        std::vector<int> validLocations;
        for (const auto& loc : specialLocations) {
            if (loc.second != avoidType && loc.first != excludeLocation) {
                validLocations.push_back(loc.first);
    ...
    int initialLocation = findValidLocation(SpecialLocationType::None);
    int forbiddenLocation = findValidLocation(SpecialLocationType::None, initialLocation);
